			...
			<form class="form-horizontal" id="package_form" method="POST" action="<?=base_url()?>admin/packages/update" enctype="multipart/form-data">
				<div class="panel-body">
					...
					<div class="form-group">
						...
						<div class="col-sm-9">
							<input type="number" name="express_interest" class="form-control" id="express_interest" value="<?=$value->express_interest?>">
						</div>
						<div class="col-sm-1">
					...
					<div class="form-group">
						...
						<div class="col-sm-9">
							<input type="number" name="direct_messages" class="form-control"  id="direct_messages" value="<?=$value->direct_messages?>">
						</div>
						<div class="col-sm-1">
					...
					<div class="form-group">
						<label class="col-sm-2 control-label" for="video_gallery"><b><?php echo translate('video_gallery')?></b></label>
						<div class="col-sm-9">
							<input type="number" class="form-control" name="video_gallery" value="<?=$value->video_gallery?>" required="">
						</div>
					</div>
...
<script>
	...
	$('#exp_int_status').click(function() {
        if ($(this).is(':checked')) {                 
           $("#express_interest").prop("disabled", true );
        } else {
           $('#express_interest').prop('disabled', false);
        }
    });
	...
	$('#dir_msg_status').click(function() {
        if ($(this).is(':checked')) {                 
           $("#direct_messages").prop("disabled", true );
        } else {
           $('#direct_messages').prop('disabled', false);
        }
     });
