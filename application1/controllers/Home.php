  ...
  function instamojo_success(){
    ...
    function login()
    {
        ...
        if ($this->member_permission() == TRUE) {
            ...
            if ($this->session->flashdata('alert') == "login_error") {
                $page_data['login_error'] = translate('Either_your_email,_mobile,_or_password_is_incorrect!');
            }
    ...
    function check_login()
    {
        if ($this->member_permission() == TRUE) {
            ...
            if(!$result){ 
                $result = $this->db->get_where('member',array('mobile'=>$username, 'password'=>$password))->row(); 
            }
